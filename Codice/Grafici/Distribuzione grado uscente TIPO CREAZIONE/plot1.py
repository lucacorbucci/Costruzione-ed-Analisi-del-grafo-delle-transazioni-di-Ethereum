...
def plotGraph(outDegree, nodesNum):
   ...
   # plt.title('Distribuzione del grado uscente nel grafo con le transazioni di creazione di contratti')
    plt.xlabel('Grado')
    plt.ylabel('Nodi')
    plt.xlim(1, 1000000)
...
def extractData():
    with open("DIROUTdegreeDistributionCREAZIONE.txt") as file:
        for line in file:
            array = line.strip("\n").split(" ")
            values = array[2:]
